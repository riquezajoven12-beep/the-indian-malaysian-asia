  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/articles" className="text-gray-400 text-sm">← Articles</Link>
        <h1 className="font-display text-xl md:text-2xl text-dark">New Article</h1>
      </div>
      <div className="bg-white rounded-xl p-5 md:p-8 shadow-sm border border-gray-100 max-w-3xl">
        <div className="flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Category</label><select className={c}>{['Politics', 'Education', 'Culture', 'Business', 'Sports', 'Community'].map(o => <option key={o}>{o}</option>)}</select></div>
            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Status</label><select className={c}><option>Draft</option><option>Published</option></select></div>
          </div>
