  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/temples" className="text-gray-400 text-sm">← Temples</Link>
        <h1 className="font-display text-xl md:text-2xl text-dark">New Temple</h1>
      </div>
      <div className="bg-white rounded-xl p-5 md:p-8 shadow-sm border border-gray-100 max-w-3xl">
        <div className="flex flex-col gap-5">
          <div><label className="block text-xs font-semibold text-gray-500 mb-1">Temple Name</label><input className={c} placeholder="Temple name..." /></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div><label className="block text-xs font-semibold text-gray-500 mb-1">State</label><select className={c}>{['Selangor', 'KL', 'Penang', 'Johor', 'Perak', 'Negeri Sembilan', 'Kedah'].map(s => <option key={s}>{s}</option>)}</select></div>
            <div><label className="block text-xs font-semibold text-gray-500 mb-1">Type</label><select className={c}><option>Hindu</option><option>Sikh</option><option>Buddhist</option></select></div>
          </div>
