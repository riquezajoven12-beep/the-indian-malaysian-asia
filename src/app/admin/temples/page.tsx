...
export default function AdminTemplesPage() {
  const temples = [
    { id: '1', name: 'Sri Mahamariamman Temple', type: 'Hindu Temple', state: 'Kuala Lumpur', verified: true },
    { id: '2', name: 'Batu Caves Temple', type: 'Hindu Temple', state: 'Selangor', verified: true },
    { id: '3', name: 'Gurdwara Sahib PJ', type: 'Sikh Gurdwara', state: 'Selangor', verified: true },
  ];
      ...
      <div style={{ display: 'flex' }}>
        <aside style={{ width: '250px', background: '#1A1A1A', minHeight: 'calc(100vh - 60px)', padding: '20px 0' }} className="admin-sidebar">
          <nav>
            ...
            <Link href="/admin/temples" style={{ display: 'block', padding: '12px 25px', color: '#FF6B00', background: 'rgba(255,107,0,0.1)', borderLeft: '3px solid #FF6B00', fontSize: '0.9rem' }}>🛕 Temples</Link>
        ...
        <main style={{ flex: 1, padding: '25px', minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px', flexWrap: 'wrap', gap: '15px' }}>
            <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.8rem', color: '#1A1A1A' }}>🛕 Temples</h1>
            <button style={{ background: 'linear-gradient(135deg, #8B1538, #D4AF37)', color: 'white', padding: '10px 20px', borderRadius: '8px', fontWeight: 600, border: 'none', cursor: 'pointer', fontSize: '0.9rem' }}>+ New Temple</button>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px', marginBottom: '25px' }}>
            <div style={{ background: 'white', padding: '20px', borderRadius: '12px' }}>
              <p style={{ color: '#888', fontSize: '0.85rem' }}>Total</p>
              <h2 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#FF6B00' }}>3,247</h2>
            </div>
            <div style={{ background: 'white', padding: '20px', borderRadius: '12px' }}>
              <p style={{ color: '#888', fontSize: '0.85rem' }}>Verified</p>
              <h2 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#2E7D32' }}>2,891</h2>
            </div>
            <div style={{ background: 'white', padding: '20px', borderRadius: '12px' }}>
              <p style={{ color: '#888', fontSize: '0.85rem' }}>Pending</p>
              <h2 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#E65100' }}>356</h2>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {temples.map((temple) => (
              <div key={temple.id} style={{ background: 'white', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px', flexWrap: 'wrap', gap: '10px' }}>
                  <h3 style={{ fontWeight: 600, fontSize: '1rem' }}>{temple.name}</h3>
