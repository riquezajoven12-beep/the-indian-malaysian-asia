  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="flex">
        ...
        {/* Sidebar */}
        <aside className={`
          fixed md:static top-[52px] left-0 h-[calc(100vh-52px)] w-60 bg-white border-r border-gray-200 z-50
          transition-transform duration-300
          ${sideOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0
          overflow-y-auto flex-shrink-0
        `}>
          <div className="px-4 py-4 border-b border-gray-200">
            <p className="text-[0.65rem] text-gray-400 font-semibold uppercase tracking-wider">Management</p>
          </div>
          <nav className="flex flex-col gap-0.5 p-2">
            {nav.map(n => (
              <Link key={n.href} href={n.href} onClick={() => setSideOpen(false)}
                className={`px-4 py-2.5 rounded-lg text-sm transition ${active(n) ? 'bg-orange-50 text-saffron font-semibold' : 'text-gray-500 hover:bg-gray-50 hover:text-dark'}`}>
                {n.label}
              </Link>
            ))}
          </nav>
